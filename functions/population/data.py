...
def get_context(raster, y, x, n):
    ...
    vals = [raster[i, j] for i in get_range(y) for j in get_range(x) if in_bounds(raster, i, j)]
...
def build_row(d, y, x, pop, rasters, feature_names, context_sizes):
    ...
    Args:
        ...
        feature_names (:obj:`list` of :obj:`np.ndarray`): name for each raster in rasters.
        context_sizes (:obj:`list` of :obj:`int`): sizes of n x n feature contexts to compute.
...
def build_dataset(params, survey_only=True):
    ...
    Args:
            ...
            survey_only (bool): True if building dataset for only sampled survey tiles.
    ...
    for roi, pop, outliers_path, zero_labels_path in zip(rois, pop_rasters, outliers_paths, zero_labels):
        ...
        d = init_cols(feature_names, context_sizes)  # initialize dictionary with column names
        ...
        for y in tqdm(range(pop.shape[0])):
            for x in range(pop.shape[1]):
                ...
                if in_zero:
                    n = 0.001  # TODO: messy workaround for log
                if n > 0 or in_zero or not survey_only:  # populate row with raster values at cell
