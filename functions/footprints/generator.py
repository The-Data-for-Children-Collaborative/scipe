...
class SatelliteImageDataGenerator(keras.utils.Sequence):
    ...
    def __getitem__(self,index):
        ...
        for i in range(self.batch_size):
            ...
            while((i_w,i_h) in self.indices): # keep generating indices until they no longer match those of a previous image
                i_w = np.random.randint(self.width-self.img_size)
                i_h = np.random.randint(self.height-self.img_size)
            self.indices.add((i_w,i_h))
            sample = self.img[i_h:(i_h+self.img_size),i_w:(i_w+self.img_size)] # take img_size x img_size crop of full image with
    ...
    def toJson(self,path):
        d = {'mean':list(self.datagen.mean.astype(float)),'std':list(self.datagen.std.astype(float))}
        with open(path,'w') as file:
            json.dump(d,file)
        print('serialized generator constants to JSON')

# Multi-image version of SatelliteImageDataGenerator, randomly chooses between both samplers each item of batch
