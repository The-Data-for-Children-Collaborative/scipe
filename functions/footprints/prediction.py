...
from tensorflow.keras.preprocessing.image import smart_resize, load_img, save_img, img_to_array

from functions.data import in_bounds
from functions.utilities import write_raster
from functions.footprints.training import sample_data, get_image_iterator
...
def estimate_footprints_full(roi,survey,img_dir,model_dirs,context_sizes,n_samples=1000):
    ...
    Estimate footprints for entire roi using specified model, and save to disk.
            ...
            Args:
                    ...
                    img_dir (str): The path to the directory containing tiles.
                    model_dir (str): Path to the model used for prediction.
                    contex_sizes (:obj:`list` of :obj:`int`): The list of context sizes to predict on around survey tiles.
                    n_samples (:obj:`int`, optional): The number of samples to use to sample featurewise mean and standard deviation.
    ...
    for i,model in enumerate(models):
        ...
        for y in tqdm(range(survey.shape[0])): # process data in vertical strips
            imgs = [(((img_to_array(load_img(f'{img_dir}{y}_{x}.tif')) / 255) - mean) / std) for x in range(survey.shape[1])]
